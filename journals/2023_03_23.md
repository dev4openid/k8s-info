...
- # Note
- There is an order ---> kubectl create deployment name image option1 option2 etc. AND/OR kubectl apply -f config-file.yaml;   so that deployments can be created/deleted/ (CRUD)